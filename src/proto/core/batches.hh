...
#include "proto/debug/logging.hh"
...
namespace proto{
    ...
    struct Entity {
        uint64_t id;
        template<typename T>
        T & add_component(T comp = T()) {
            return Batch<T>::create_component(*this, comp);
        }

        template <typename T>
        proto::optional<T> get_component() {
            return Batch<T>::get_component(*this);
        }

        template <typename T>
        proto::optional<T*> get_component_mut() {
            return Batch<T>::get_component_mut(*this);
        }
    };
    ...
    struct DrawableMesh_Comp : Component{
        Transform_Comp transform;
        proto::Mesh2D<Vertex> * mesh;
    };

    template<typename T>
    struct BasicBatch {
        using component_t = T;
        using allocator_t = proto::memory::linear_allocator;
        inline static allocator_t _allocator {};
        inline static proto::dynamic_array<Entity, allocator_t> keys {};
        inline static proto::dynamic_array<component_t, allocator_t> components {};
        ...
        static proto::optional<component_t> get_component(Entity entity) {
            for(size_t i=0; i < keys._size; i++)
                if(keys[i].id == entity.id)
                    return proto::optional<component_t>{.value = components[i],
                                                         .has_value = true};

            assert(1);
            return proto::optional<component_t>{};
        }

        static proto::optional<component_t*> get_component_mut(Entity entity) {
            for(size_t i=0; i < keys._size; i++)
                if(keys[i].id == entity.id)
                    return proto::optional<component_t*>{.value = &components[i],
                                                        .has_value = true};
    ...
    struct DebugLine_Comp {
        glm::vec2 from;
        glm::vec2 to;
    };
    template<>
    struct Batch<DebugLine_Comp> {

        using component_t = DebugLine_Comp;
        using allocator_t = proto::memory::linear_allocator;
        inline static allocator_t _allocator {};
        inline static proto::dynamic_array<Entity, allocator_t> keys {};
        inline static proto::dynamic_array<component_t, allocator_t> components {};
        //inline static proto::Shader debugline_shader;
        inline static GLuint debugline_VAO, debugline_VBO;

        static int init() {
            _allocator.init(proto::memory::megabytes(1));
            keys.init(1024, &_allocator);
            components.init(1024, &_allocator);

            debugline_VAO = debugline_VBO = 0;
            ...
            //debugline_shader = Shader(shader_vert_src, shader_frag_src);
            return 0;
        }

        static void process() {glBindVertexArray(debugline_VAO); glBindBuffer(GL_ARRAY_BUFFER, debugline_VBO);

            glBufferData(GL_ARRAY_BUFFER,
                         components.size() * sizeof(DebugLine_Comp),
                         components.raw(), GL_DYNAMIC_DRAW);

            debugline_shader.use();
            glDrawArrays(GL_LINES, 0, components.size() * 2);
        }

    };
    /***********************************************************************
TEXT REDNERING SUPER EARLY, MAKE GLYPH TEXTURE ATLAS
     */
    ...
    void create_ascii_font_atlas(ASCIIFontAtlas * ret, int8_t font_size) {
        ...
        if(FT_New_Face(ft, "arial.ttf", 0, &face))
            debug_warn(proto::debug::graphics,
                       "failed to load font");
    ...
    template<> struct Batch<TextRect_Comp> : BasicBatch<TextRect_Comp>{
        //inline static proto::Shader text_shader;
        ...
        static int init() {
            _allocator.init(proto::memory::megabytes(2));
            ...
            //text_shader = proto::Shader(text_vert_shader, text_frag_shader);
...
TEXT RENDERING END HERRE
    ...
    template<>
    struct Batch<Transform_Comp> : BasicBatch<Transform_Comp>{
        static int init() {
            _allocator.init(proto::memory::megabytes(2));
            keys.init(256, &_allocator);
            components.init(256, &_allocator);
    ...
    template<>
    struct Batch<DrawableMesh_Comp> : BasicBatch<DrawableMesh_Comp> {
        ...
        static void process() {
            for(size_t i=0; i<components.size(); i++) {
            //     mesh_shader.use();
            //     mesh_shader.set_uniform("transform_position", GL_FLOAT_VEC3,
            //                             &components[i].transform.position.x);
            //     components[i].mesh->bind();
            //     components[i].mesh->draw();
            }
        }
